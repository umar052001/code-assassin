...
const ProjectCard = ({
  ...
  const projectId = `${title.charAt(0).toLowerCase()}${title
    .slice(1)
    .replace(/\s/g, "")}`;
  ...
  const toggleCardModal = () => {
    ...
    const state = Flip.getState(
      ".projects.dev .projects-container, .projects.dev .project-card"
    );
      ...
      if (textContainerRef.current) {
        ...
        if (textOverflowsContainer) {
          textContainerRef.current.style.setProperty(
            "--maxlines",
            getMaxLines(textContainerRef.current)
          );
        }
      }
    };
        ...
        <div className="card-back">
          ...
          <ul className="card-tools">
            {tools &&
              tools.map((tool) => {
                return (
                  <li className="card-tool" key={tool}>
                    {tool}
                  </li>
                );
              })}
